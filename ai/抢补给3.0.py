...
def update(board):
    ...
    def path_to_for_cal_danger_level(start, end, special_pos):
        grid = [[0] * 8 for _ in range(8)]
        ...
        grid[start[0]][start[1]] = 0
        grid[end[0]][end[1]] = 0
        grid[special_pos[0]][special_pos[1]] = 0
    ...
    def cal_danger_level(pos, my_bot):
        danger_level = 0
        for enm in enms:
            if enm.id == 0:
                continue
            else:
                danger_level += (cal_distance(pos, get_pos(enm)) if enm.type == 'archer' else (len(path_to_for_cal_danger_level(get_pos(enm), pos, get_pos(my_bot))) - 1))
        return danger_level
    ...
    def enter_more_danger_pos(now_pos, next_pos):
        now_danger_level = 0
        for enm in enms:
            if enm.id != 0 and now_pos in get_max_attack_range(enm):
                now_danger_level = enm.attack_strength
        for enm in enms:
            if enm.id != 0 and next_pos in get_max_attack_range(enm) and enm.attack_strength > now_danger_level:
                return 1
        return 0
        ...
        if get_pos(my_bot) in supplies:
            if len(get_attackable_bots_in_move_range(my_side, my_bot)) == 0:
                ...
                for enm in atk_enms:
                    ...
                    if stay_in_supplies_and_can_attack:
                        break
                if len(stay_in_supplies_and_can_attack) != 0:
                    tmp_range = [x for x in stay_in_supplies_and_can_attack if not enter_more_danger_pos(my_bot_pos, x)]
                    target_pos = None
                    if tmp_range:
                        target_pos = tmp_range[0]
                    else:
                        target_pos = stay_in_supplies_and_can_attack[0]
                        max_danger_level = cal_danger_level(target_pos, my_bot)
                        for pos in stay_in_supplies_and_can_attack:
                            now_danger_level = cal_danger_level(pos, my_bot)
                            if now_danger_level > max_danger_level:
                                target_pos = pos
                                max_danger_level = now_danger_level
                        
                    next_move_pos, flag1 = get_next_move_pos(my_bot, target_pos)
                    if my_bot.type != 'protector':
    ...
    def arc_attack():
        ...
        for pos in move_range:
            if (realm.get_chess_details_by_pos(board.layout, pos) == None or get_pos(arc) == pos) and atk_enm[0] in get_attackable_bots(my_side, arc, pos):
                now_danger_level = cal_danger_level(pos, arc)
                now_safe_level = cal_safe_level(pos)
                if now_danger_level > max_danger_level or \
                (max_danger_level == now_danger_level and target_pos in get_bot_attack_range(enm_side, atk_enm[0], get_pos(atk_enm[0])) and pos not in get_bot_attack_range(enm_side, atk_enm[0], get_pos(atk_enm[0]))) or \
                (max_danger_level == now_danger_level and target_pos not in get_bot_attack_range(enm_side, atk_enm[0], get_pos(atk_enm[0])) and pos not in get_bot_attack_range(enm_side, atk_enm[0], get_pos(atk_enm[0])) and cal_safe_level(pos) < cal_safe_level(target_pos)):
                    target_pos = pos
                    max_danger_level = now_danger_level
